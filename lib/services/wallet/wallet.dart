...
Future<WalletService?> walletServiceFromCredentials(
  BigInt chainId,
  Wallet creds,
) async {
  final List rawNativeChains =
  ...
  final i = nativeChains.indexWhere((c) => c.chainId == chainId.toInt());
  if (i >= 0) {
    return WalletService.fromCredentials(
      nativeChains[i],
      creds,
    );
  }
...
class WalletService {
  ...
  /// creates using a wallet file from credentials
  /// init before using
  WalletService.fromCredentials(
    this._chain,
    Wallet creds,
  ) {
    final url = _chain!.rpc.first;
